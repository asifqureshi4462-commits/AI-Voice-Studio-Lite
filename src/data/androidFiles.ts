      ...
      - name: Ensure Valid Gradle Wrapper JAR
        run: |
          ...
          if [ ! -f "$JAR_PATH" ] || [ $(wc -c < "$JAR_PATH") -lt 10000 ]; then
            echo "Downloading valid Gradle Wrapper JAR..."
            curl -sSL -o "$JAR_PATH" https://raw.githubusercontent.com/gradle/gradle/v8.5.0/gradle/wrapper/gradle-wrapper.jar
          fi
          echo "Gradle Wrapper JAR size: $(wc -c < "$JAR_PATH") bytes"
